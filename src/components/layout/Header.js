...
import { emptyPortfolioContent, navLinks } from "@/data/portfolioData";
...
export default function Header({ profile = emptyPortfolioContent.profile }) {
  ...
  const showThreadList = userChatView === "threads";
  ...
  const adminChatController = useAdminChatController({
    enabled: Boolean(authUser?.id) && isAuthApiEnabled() && isAdmin,
    token: getAuthToken(),
    ...
    isOpen: messageOpen,
    resetToListOnOpen: true,
  });

  const effectiveUnreadBadgeText = formatUnreadCount(
    isAdmin ? adminChatController.unreadCount : unreadCount
  );
  ...
  }, [authUser, chatMessages, isAdmin, isPinnedToBottom, messageOpen, userChatView]);
  ...
  const handleSaveEditMessage = async (messageId) => {
    ...
    const response = await editUserChatMessage({
      token,
      messageId,
  ...
  const handleDeleteMessage = async (messageId) => {
    ...
    setChatActionPendingId(messageId);
    const response = await deleteUserChatMessage({
      token,
      messageId,
    });
    setChatActionPendingId("");
  ...
  const handlePickEmoji = async (messageId, emoji) => {
    ...
    setChatActionPendingId(messageId);
    const response = await reactUserChatMessage({
      token,
      messageId,
      emoji: nextEmoji,
    });
          ...
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div className="flex min-w-0 items-center gap-2">
              {userChatView === "thread" ? (
                <button
                  type="button"
                  onClick={handleBackToUserThreads}
          ...
          <div className="relative flex-1 min-h-0">
            {showThreadList ? (
              ...
              <>
                ...
                const isIncomingMessage = item.sender !== "you";
